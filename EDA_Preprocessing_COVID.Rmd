...
Continent <- data %>% select(Continent_Name, Country_Name, Two_Letter_Country_Code) %>% 
  mutate(iso = str_to_lower(Two_Letter_Country_Code)) %>% select(-Two_Letter_Country_Code)
...
roaming_data_new <-  roaming_data_new %>% mutate(stay_time = ymd(departure)-ymd(arrival),
